...
/**
 * BeanPostProcessor是Spring框架提供的一个扩展类点之一（不止一个）
 * 通过实现BeanPostProcessor接口，程序员就可插手Bean实例化的过程,在实例化前后实现一些逻辑，从而减轻了beanFactory的负担
 * 值得说明的是这个接口可以设置多个（实现类），会形成一个列表，然后按优先级依次执行
 * (自己实现的BeanPostProcessor加个@Component注解，spring会自动扫描并加载到容器中，
 * 但是spring默认的BeanPostProcessor怎么办？默认的是没有加注解，spring是通过手动set)
 *
 ...
 * 可以来演示一下 BeanPostProcessor 的使用方式（把动态代理和IOC、aop结合起来使用），如TestBeanPostProcessor，TestBeanPostProcessor1
 ...
 *    acap后置处理器的作用是，比如：当应用程序定义的Bean实现ApplicationContextAware接口时给Bean注入ApplicationContext对象
